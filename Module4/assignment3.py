...
#
# .. your code here ..
used = ['rbc', 'pc', 'pcc', 'ba', 'htn', 'dm',
           'cad', 'appet', 'pe', 'ane']
           
notused = ['id', 'classification']

notused2 = ['id', 'classification','rbc', 'pc', 'pcc', 'ba', 'htn', 'dm',
           'cad', 'appet', 'pe', 'ane']

df = df.drop(labels=notused2, axis=1)

df.pcv = pd.to_numeric(df.pcv, errors='coerce')
df.wc = pd.to_numeric(df.wc, errors='coerce')
df.rc = pd.to_numeric(df.rc, errors='coerce')

#for use in used: df = pd.get_dummies(df,columns=[use])
