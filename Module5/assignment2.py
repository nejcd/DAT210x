...
def explore(df, userID):
    
    
    user1 = df[df.In == userId]
    ...
    #
    # .. your code here ..
    user1_geo = user1.loc[:,['TowerLat', 'TowerLon']]
      
    kmeans = KMeans(n_clusters=2)
    kmeans.fit(user1_geo)
    KMeans(copy_x=True, init='k-means++', max_iter=300, n_clusters=2, n_init=10,
       n_jobs=1, precompute_distances='auto', random_state=None, tol=0.0001,
       verbose=0)
    
    labels = kmeans.predict(user1_geo)
    centroids = kmeans.cluster_centers_
    
    #
    # INFO: Print and plot the centroids...
    #centroids = kmeans_model.cluster_centers_
    ax.scatter(user1_geo.TowerLon, user1_geo.TowerLat, marker='.', alpha=0.3, c=labels)
...
for n in range(0,10):
    print ('User {0}, phone number {1}'.format(n, users[n]))
    userId = int(users[n])
    explore(df, userId)
